...
<!-- Time-stamp: "2020-06-22T15:19:10Z" -->
...
やること 6 つ目、もし、街角に掲示するなどのため、特定の IP からのみ書
...
そこにアクセスすると、シンプルなページが表示され、ファイルを指定すると
ころと、送信ボタンのみがあります。log.html を指定してから送信すると、
結果が次のページに表示され、Result の最後に OK が出て「チェック済みの
メモ」が出力されればほぼ偽造がないと判断できます。
...
シュを取った Sum Hash のチェックが表示されます。これは、すべてのメッセー
ジに ok が表示された上での並び方のチェックとしての意味があります。
